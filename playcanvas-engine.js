...
function SetUpPlayCanvas(){
	...
	pc = require('./public/playcanvas-latest-mod.js').pc;
...
function CreateScene0(){
	...
	//setTimeout(function(){
		...
		simplescript.addComponent('script',{
			url:'http://127.0.0.1/client/scriptbase.js'
		});
	...
	app.on("update", function (dt) {
		...
		if(OBJIONetworkType == 1){
			...
			if(typeof engineiobroadcast == 'function'){
				...
				//console.log(ball.entity.position.x);
				var data = new Sceneobj({
					type:"ball",
					//name:'none',
					//id:'none',
					px:ball.entity.position.x,
					py:ball.entity.position.y,
					pz:ball.entity.position.z,
					rx:ball.entity.rotation.x,
					ry:ball.entity.rotation.y,
					rz:ball.entity.rotation.z,
					rw:ball.entity.rotation.w
				});
				//console.log(data);
				engineiobroadcast(data.toArrayBuffer()); //create this function to send out each client
				//engineiobroadcast(data); //create this function to send out each client
			...
			if(OBJIONetworkType == 1){
				if(typeof engineiobroadcast == 'function'){
					...
					var data = new Sceneobj({
						...
						//name:'none',
						id:i.toString(),
						px:blocks[i].position.x,
						py:blocks[i].position.y,
						pz:blocks[i].position.z,
						rx:blocks[i].rotation.x,
						ry:blocks[i].rotation.y,
						rz:blocks[i].rotation.z,
						rw:blocks[i].rotation.w
					});
					engineiobroadcast(data.toArrayBuffer()); //create this function to send out each client
				}
			}
